...
alter table stock add constraint fk_stock_item_7 foreign key (item_id) references item (id) on delete restrict on update restrict;
create index ix_stock_item_7 on stock (item_id);
alter table wishlist_item add constraint fk_wishlist_item_item_8 foreign key (item_id) references item (id) on delete restrict on update restrict;
create index ix_wishlist_item_item_8 on wishlist_item (item_id);
alter table wishlist_item add constraint fk_wishlist_item_owner_9 foreign key (owner_user_id) references user (user_id) on delete restrict on update restrict;
create index ix_wishlist_item_owner_9 on wishlist_item (owner_user_id);
