...
public class ItemController extends BaseController {
	...
	public static Result submit() {
		...
		if (itemFilledForm.hasErrors()) {
			...
			return ok(item.render(Item.submit(itemFilledForm.get())));
		}
	}
	...
	/***
	 * Returns a list of all items in Json. ownerUser: if present and != null,
	 * list only contains items in which its seller fbuserid is equal to the one
	 * in ownerUser
	 */
	...
	/***
	 * Updates (and if does not exists creates) an item. All params are
	 * optionally, only the ones present in the request will be updated.
	 * 
	 * @return 201 Created: if item already existed, 200 OK: if item didn't
	 *         exist and was created.
	 */
	@RestrictTo(Roles.SELLER)
	public static Result createOrUpdate(String id) {
		Item item = Item.find.byId(id); // TODO validate item owner is current
										// user, if not return forbiden
	...
	public static Result delete(String itemId) {
		Item item = Item.find.byId(itemId); // TODO validate item owner is
											// current user, if not forbiden
		if (item != null) {
			item.deleteCascade();
			return noContent();
		} else {
	...
	public static Result itemSearch() {
		...
		List<Item> items = Item.find.where().ilike("title", "%" + textSearch + "%").findList();
		return ok(Json.toJson(items));
	}
	...
	public static Result buy() {
		...
		if (item != null) {
			...
			// TODO should be 422 as it's a smantic error not sintax. Does plays
			// allow to return a 422?
		}
	}
	...
	public static Result orderItem(String itemId, String size, Integer pointsUsed) throws Exception {
		...
		try {
			...
			jsonResult.put("checkoutUrl", manager.checkout(buyOrder));
			jsonResult.put("buyOrderId", buyOrder.id);
